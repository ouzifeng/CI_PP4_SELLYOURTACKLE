...
from .forms import CustomUserSignupForm, ContactForm, PasswordResetRequestForm, SetNewPasswordForm
...
class CustomUserSignupFormTest(TestCase):
    ...
    def test_signup_form_invalid(self):
        form_data = {'email': 'invalid', 'password1': 'password123', 'password2': 'password124'}
        form = CustomUserSignupForm(data=form_data)
...
class ContactFormTest(TestCase):
    ...
    def test_contact_form_valid(self):
        form_data = {'name': 'Test Name', 'email_address': 'test@example.com', 'message': 'Test message'}
...
class ConfirmEmailViewTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(email='testuser@example.com', password='password123', is_active=False)
        self.token = '3e65560a-187d-44d1-af64-500162fe305c'  

    def test_confirm_email(self):
        token = EmailConfirmationToken.objects.create(user=self.user, token=self.token)
        response = self.client.get(reverse('confirm-email', args=[self.user.id, self.token]))
...
class BuyingViewTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(email='testbuyer@example.com', password='password123')
        self.client.login(email='testbuyer@example.com', password='password123')
...
class StripeIntegrationTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(email='teststripe@example.com', password='password123', stripe_account_id='acct_123')
    ...
    def test_handle_stripe_return(self, mock_retrieve, mock_account_link):
        ...
        self.client.login(email='teststripe@example.com', password='password123')
